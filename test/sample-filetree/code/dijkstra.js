function main() {
  ...
  const src = Number.parseInt(process.argv[3]);
  const des = Number.parseInt(process.argv[4]);

  usage =
    "Usage: please provide three inputs: a serialized matrix, a source node and a destination node";

  if (Number.isNaN(des) || Number.isNaN(src) || !mat) {
  ...
  while (matList.length) {
    mapping.push(matList.splice(0, n).map((i) => Number.parseInt(i)));
  }
  ...
  const dijkstra = (src, des, n) => {
    ...
    const dist = new Array(n).fill(Number.Infinity);
